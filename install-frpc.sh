...
shell_update() {
	...
	if ! wget --no-check-certificate -qO "$0" "${str_install_shell}"; then
		...
		echo -e "${COLOR_GREEN}Please re-run${COLOR_END} ${COLOR_PINK}$0 ${frpc_action}${COLOR_END}"
		echo
		exit 1
	fi
    else
...
check_os_bit() {
    ...
    case $arch in
        ...
        i386|i486|i586|i686) Is_64bit='n'; ARCHS="386"; FRPC_VER="$FRPC_VER_32BIT";;
        aarch64)     Is_64bit='y'; ARCHS="arm64";;
        arm*|armv*)  Is_64bit='n'; ARCHS="arm"; FRPC_VER="$FRPC_VER_32BIT";;
        mips)        Is_64bit='n'; ARCHS="mips"; FRPC_VER="$FRPC_VER_32BIT";;
        mips64)      Is_64bit='y'; ARCHS="mips64";;
        mips64el)    Is_64bit='y'; ARCHS="mips64le";;
        mipsel)      Is_64bit='n'; ARCHS="mipsle"; FRPC_VER="$FRPC_VER_32BIT";;
...
fun_getVer(){
    ...
    LATEST_RELEASE=$(curl -s ${github_latest_version_api} | grep '"tag_name":' | cut -d '"' -f 4 | cut -c 2-);
...
pre_install_frpc(){
    ...
    case "$choice" in
      ...
      *)
        ...
        echo -e ""
        fun_input_host
        [ -n "${input_host}" ] && set_host="${input_host}"
        echo -e "${program_name} host: ${COLOR_YELOW}${set_host}${COLOR_END}"
        echo -e ""
        ...
        echo -e "You Server IP      : ${COLOR_GREEN}${set_host}${COLOR_END}"
...
# Write the configuration to the frpc config file
...
#
	...
	if [ ! -s ${program_init} ]; then
		if ! wget  -q ${FRPC_INIT} -O ${program_init}; then
			echo -e " ${COLOR_RED}failed${COLOR_END}"
			exit 1
		fi
	fi
...
update_config_frpc(){
    if [ ! -r "${str_program_dir}/${program_config_file}" ]; then
        ...
        if [ -z "${search_dashboard_user}" ] || [ -z "${search_dashboard_pwd}" ]  || [ ! -z "${search_token}" ];then
        ...
        if [ ! -z "${verify_dashboard_user}" ] && [ ! -z "${verify_dashboard_pwd}" ] && [ -z "${verify_token}" ];then
...
update_program_server_frpc() {
    ...
    if [ -s "$program_init" ] || [ -s "$str_program_dir/$program_name" ]; then
        ...
        remote_init_version=$(wget -qO- "$FRPC_INIT" | sed -n '/^version/p' | cut -d\" -f2)
        ...
        if [ -n "$remote_init_version" ]; then
            if [ "$local_init_version" != "$remote_init_version" ]; then
                ...
                if ! wget "$FRPC_INIT" -O "$program_init"; then
...
if [ -z "$action" ]; then
    ...
    case "$action" in
    install)
        pre_install_frpc 2>&1 | tee /root/${program_name}-install.log
        ;;
    config)
        configure_program_server_frpc
        ;;
    uninstall)
        uninstall_program_server_frpc 2>&1 | tee /root/${program_name}-uninstall.log
        ;;
    update)
        update_program_server_frpc 2>&1 | tee /root/${program_name}-update.log
        ;;
    *)
        fun_frpc
        echo "Arguments error! [$action ]"
